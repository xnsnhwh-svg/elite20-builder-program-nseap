...
本模块借鉴的是成熟知识产品的能力模式，而不是照搬某一个通用知识库。

| 竞品方向 | 可借鉴能力 | 在本项目中的落地方式 |
| --- | --- | --- |
| 飞书知识库、语雀、Notion、Confluence | 文档协作、模板、搜索、快速上手 | 保留清晰的知识类型、三步上传流程、metadata 编辑和中文工作台表达 |
| Dify、RAGFlow | 文档上传、正文抽取、分块、RAG 检索、接入 Agent | 上传资料后先抽取正文，再生成知识草稿，并预留 LLM 分类和 Agent 检索接口 |
| Guru | 知识治理、可信状态、重复/过期/缺口管理 | 用 `draft / review / stable / archived` 表达知识生命周期，删除改为归档 |
| Obsidian、Heptabase | 双向链接、知识关系、学习路径 | 用 `relationships` 显式表达课程、挑战、项目、Prompt、技能之间的关系 |

这些竞品大多解决“通用文档管理”或“通用 AI 检索”，但不会天然理解 NSEAP 的课程、挑战、项目案例、Prompt、FAQ 和 Agent 上下文结构。因此本模块的差异点不是做一个更大的文档库，而是把真实项目资料沉淀成适合教学复用、项目复盘和 Agent 调用的结构化知识资产。
...
浏览模块的目标不是让用户自己翻资料，而是帮助用户快速掌握知识并完成复用。因此详情页应提供“老师带练”入口，把一条知识扩展成可跟随的学习路径。

```text
用户打开一条知识
-> 点击开始带练
-> 系统根据 type 和 relationships 生成 3-5 步路径
-> 每一步提供老师讲解、当前知识卡片和小检查问题
-> 用户完成路径后形成下一步行动或复用思路
```

示例：
用户打开一个 `challenge` 条目时，带练路径可以是：
...
当前 MVP 先使用规则模板生成带练路径。后续在配置 LLM Key 后，可以把每一步的 `teacherScript` 和 `checkQuestion` 升级为模型生成，让讲解更贴合具体内容。
...
7. Word / PDF 正文抽取是否能让真实资料进入自动分类流程。
8. LLM 配置是否能作为后续自动分类能力的接入点。
9. 归档机制是否能替代硬删除，保留审计和恢复空间。
...
| 搜索悬浮预览 | 搜索结果 metadata preview |
| 知识图谱视图 | `GET /api/knowledge-graph` |
| 开始带练按钮 | `GET /api/guided-path?entryId=...` |
| 详情面板 | `GET /api/knowledge-items/{id}` |
| 关系展示 | `knowledge_relationships` |
| 上传资料 | `POST /api/upload` + 原始文件存储 |
| docx/pdf 正文抽取 | 文件解析服务 / ingestion pipeline |
| 草稿 metadata 编辑 | `PATCH /api/knowledge-items/{id}` |
| 关系维护 | `GET/POST /api/knowledge-items/{id}/relationships` |
| 归档确认按钮 | `DELETE /api/knowledge-items/{id}` 将状态改为 `archived` |
